    ...
    C1: ShareConvert<P256> + Flush + Send + 'static,
{
    ...
    async fn compute_shares(&mut self, ctx: &mut Context) -> Result<(), KeyExchangeError> {
        ...
        println!("[KEY EXCHANGE] {} private key: {:?}", entity_name, self.private_key);
        ...
        println!("[KEY EXCHANGE] {} PMS share 0: {:02x?}", entity_name, pms_0.to_be_bytes());
        println!("[KEY EXCHANGE] {} PMS share 1: {:02x?}", entity_name, pms_1.to_be_bytes());
    ...
    fn assign(&mut self, vm: &mut dyn Vm<Binary>) -> Result<(), KeyExchangeError> {
        ...
        println!("[KEY EXCHANGE] Assigning key shares to VM for {}", entity_name);
        println!("[KEY EXCHANGE] {} Share 0: {:02x?}", entity_name, share_0_bytes);
        println!("[KEY EXCHANGE] {} Share 1: {:02x?}", entity_name, share_1_bytes);
