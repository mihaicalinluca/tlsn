    ...
    <C as MultiplicativeToAdditive<P256>>::Future: Send,
{
    ...
    println!(
        "[KEY DERIVATION] Deriving x-coordinate share for {}",
        entity_name
    );
    println!(
        "[KEY DERIVATION] {} Input point: {:02x?}",
        entity_name,
        share.as_bytes()
    );
    ...
    println!(
        "[KEY DERIVATION] {} Decomposed point - x: {:02x?}, y: {:02x?}",
        entity_name,
        x.to_be_bytes(),
        y.to_be_bytes()
    );
    ...
    println!(
        "[KEY DERIVATION] {} Processed shares a: {:02x?}, b: {:02x?}",
        entity_name,
        a.to_be_bytes(),
        b.to_be_bytes()
    );
    ...
    println!(
        "[KEY DERIVATION] {} Final x_r share: {:02x?}",
        entity_name,
        x_r.to_be_bytes()
    );
